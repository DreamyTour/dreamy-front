...
function getToursByPageCategory(page: Page, tours: Tour[]) {
  const pageDestinationCategory =
    page?.categories?.find((category: PageCategory) => category.slug !== "peru-tours") ??
  ...
  return tours.filter((tour) =>
    tour.categories?.some(
      (category: TourCategory) => category.slug === pageDestinationCategory.slug,
    ),
  );
}

export async function getDefaultLangDynamicPaths(): Promise<DefaultRoutePath[]> {
...
export async function getLocalizedDynamicPaths(): Promise<LocalizedRoutePath[]> {
  ...
  for (const lang of LANGS) {
    ...
    const currentSlugs = new Set(currentLangTours.map((tour: Tour) => tour.slug));
